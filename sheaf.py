    ...
    def forward(self, features, qmask, umask, shift_pred, prev, valid):
        ...
        if self.per_modal:
            ...
            for m, stack in enumerate(self.layers):
                sl = slice(m * T, (m + 1) * T)
                ei, er, eb = to_edge_list(rel[:, sl, sl], both[:, sl, sl], drop_self=True)
                xm = features[m].reshape(B * T, self.d, self.f)
                for layer in stack:
                    xm = layer(xm, ei, er, eb, B * T)
                outs.append(xm.reshape(B, T, H))
            return outs
...
class EmotionalGATGraph(nn.Module):
    """
    per_modal=False : one GAT over all M*T nodes, weights shared across modalities
    per_modal=True  : one GAT per modality, each with its own weights (needs inter_modal=False,
                      so that the graph is block diagonal). The partition, and therefore the
                      edge set, is still shared: ShiftHead reads h = sum(hs).
    """
